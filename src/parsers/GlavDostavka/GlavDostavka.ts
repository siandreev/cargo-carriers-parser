...
import "parsers/GlavDostavka/request.json";
...
class GlavDostavka extends Parser {
    ...
    static async createFormData(): Promise<string> {
        const request = JSON.parse(await global.readFile('dist/parsers/GlavDostavka/request.json', 'utf8'));
        const parsed: any = {};
    ...
    private async getCityId(cityName: string): Promise<number> {
        ...
        let citiesInfo: CitiesInfo = JSON.parse(json);
        ...
        if (this.mustRefresh(citiesInfo.timestamp)) {
            ...
            global.writeFile(JSON.stringify(citiesInfo))
        }
